...
az acr build    -r acrslotslab -t docprocessor:v1 ./dotnet
...
az webapp          create -n docprocessor-slots-001 -g rg-slots-lab -p plan-slots-lab  \
  -c acrslotslab.azurecr.io/docprocessor:v1
...
az webapp deployment slot create -n docprocessor-slots-001 -g rg-slots-lab  -s staging
...
az webapp identity assign -n docprocessor-slots-001 -g rg-slots-lab -s staging
principalIdStaging=$(az webapp identity show -n docprocessor-slots-001 -g rg-slots-lab -s staging --query principalId -o tsv)
...
az webapp config set -n docprocessor-slots-001 -g rg-slots-lab -s staging --generic-configurations '{"acrUseManagedIdentityCreds": true}'

# Sin el parametro -s (--slot) → va al slot de producción (el implícito)
...
az webapp config appsettings set  -n docprocessor-slots-001 -g rg-slots-lab -s staging --settings APP_VERSION="1.1.0"
az webapp config appsettings set -n docprocessor-slots-001 -g rg-slots-lab -s staging --slot-settings ENVIRONMENT=staging

# Actualizar staging con una nueva versión de la imagen
az acr build -r acrslotslab -t docprocessor:v2 ./dotnet
az webapp config container set -n docprocessor-slots-001 -g rg-slots-lab -s staging \
  -c acrslotslab.azurecr.io/docprocessor:v2

az webapp config container show -n docprocessor-slots-001 -g rg-slots-lab
az webapp config container show -n docprocessor-slots-001 -g rg-slots-lab -s staging

# Swap: (staging) slot de origen -> (production) slot de destino
# -s (--slot) staging      -> 	El slot de origen
# --target-slot production ->   El slot de destino (sin forma corta)
az webapp deployment slot swap -n docprocessor-slots-001 -g rg-slots-lab -s staging --target-slot production
