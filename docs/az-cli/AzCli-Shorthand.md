...
| `acr build` | `--image` | `-t` |
| `acr repository show` | `--name` | `--name` *(sin corto)* |
| `acr repository show` | `--image` | *(sin corto)* |
...
| `--schedule` | *(sin corto)* |
| `--context` | *(sin corto)* |
| `--image` | *(sin corto en `task create`, a diferencia de `acr build`)* |

⚠️ **Recuerda:** `--image` en `az acr build` **sí** tiene corto (`-t`), pero en `az acr task create` **no** — hay que escribirlo completo.
...
| `-t` | `--image` | Solo `az acr build` (¡no en `acr task create`!) |
